...
changed, added = cd(ENV["TRAVIS_BUILD_DIR"]) do
    ...
    # a string containing a newline-delimited list of files changed
    _changed = readchomp(`git diff --name-only $PR_COMMIT_SHA $upstream_commit`)
    _added = readchomp(`git diff --name-only --diff-filter=A $PR_COMMIT_SHA $upstream_commit`)
